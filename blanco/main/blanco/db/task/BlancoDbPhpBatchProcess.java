...
/**
 * バッチ処理クラス [BlancoDbPhpBatchProcess]。
 *
 * <P>バッチ処理の呼び出し例。</P>
 * <code>
 * java -classpath (クラスパス) blanco.db.task.BlancoDbPhpBatchProcess -help
 * </code>
 */
public class BlancoDbPhpBatchProcess {
    ...
    public static final void main(final String[] args) {
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (arg.startsWith("-verbose=")) {
                ...
                System.out.println("BlancoDbPhpBatchProcess: 入力パラメータ[" + arg + "]は無視されました。");
                isNeedUsage = true;
            }
        }
        ...
        if( isFieldJdbcdriverProcessed == false) {
            System.out.println("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[jdbcdriver]に値が設定されていません。");
            System.exit(END_ILLEGAL_ARGUMENT_EXCEPTION);
        }
        if( isFieldJdbcurlProcessed == false) {
            System.out.println("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[jdbcurl]に値が設定されていません。");
            System.exit(END_ILLEGAL_ARGUMENT_EXCEPTION);
        }
        if( isFieldJdbcuserProcessed == false) {
            System.out.println("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[jdbcuser]に値が設定されていません。");
            System.exit(END_ILLEGAL_ARGUMENT_EXCEPTION);
        }
        if( isFieldJdbcpasswordProcessed == false) {
            System.out.println("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[jdbcpassword]に値が設定されていません。");
            System.exit(END_ILLEGAL_ARGUMENT_EXCEPTION);
        }
        if( isFieldMetadirProcessed == false) {
            System.out.println("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
            System.exit(END_ILLEGAL_ARGUMENT_EXCEPTION);
        }
        if( isFieldBasepackageProcessed == false) {
            System.out.println("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[basepackage]に値が設定されていません。");
            System.exit(END_ILLEGAL_ARGUMENT_EXCEPTION);
        }
    ...
    public final int execute(final BlancoDbPhpProcessInput input) throws IllegalArgumentException {
        try {
            ...
            System.out.println("BlancoDbPhpBatchProcess: 入力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_ILLEGAL_ARGUMENT_EXCEPTION;
        } catch (IOException ex) {
            System.out.println("BlancoDbPhpBatchProcess: 入出力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_IO_EXCEPTION;
        } catch (RuntimeException ex) {
            System.out.println("BlancoDbPhpBatchProcess: ランタイム例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            ex.printStackTrace();
            // 異常終了。
            return END_ERROR;
        } catch (Error er) {
            System.out.println("BlancoDbPhpBatchProcess: ランタイムエラーが発生しました。バッチ処理を中断します。:" + er.toString());
            er.printStackTrace();
            // 異常終了。
            return END_ERROR;
        }
    ...
    public static final void usage() {
        ...
        System.out.println("  java blanco.db.task.BlancoDbPhpBatchProcess -verbose=値1 -jdbcdriver=値2 -jdbcurl=値3 -jdbcuser=値4 -jdbcpassword=値5 -jdbcdriverfile=値6 -metadir=値7 -tmpdir=値8 -targetdir=値9 -basepackage=値10 -runtimepackage=値11 -schema=値12 -table=値13 -sql=値14 -log=値15 -executesql=値16 -encoding=値17 -cache=値18");
        ...
        System.out.println("      説明[BlancoDbがPHPソースコードを出力するディレクトリを指定します。]");
        System.out.println("      型[文字列]");
        System.out.println("      デフォルト値[blanco.php]");
        System.out.println("    -basepackage");
        System.out.println("      説明[BlancoDbがPHPソースコードを生成する際の基準となるパッケージ名を指定します。]");
        System.out.println("      型[文字列]");
        System.out.println("      必須パラメータ");
        ...
        System.out.println("      説明[Trueを設定するとロギングコードを生成します。※現時点ではサポートされないオプションです。]");
        System.out.println("      型[文字列]");
        System.out.println("      デフォルト値[false]");
    ...
    public void validateInput(final BlancoDbPhpProcessInput input) throws IllegalArgumentException {
        ...
        if (input.getJdbcdriver() == null) {
            throw new IllegalArgumentException("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[jdbcdriver]に値が設定されていません。");
        }
        if (input.getJdbcurl() == null) {
            throw new IllegalArgumentException("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[jdbcurl]に値が設定されていません。");
        }
        if (input.getJdbcuser() == null) {
            throw new IllegalArgumentException("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[jdbcuser]に値が設定されていません。");
        }
        if (input.getJdbcpassword() == null) {
            throw new IllegalArgumentException("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[jdbcpassword]に値が設定されていません。");
        }
        if (input.getMetadir() == null) {
            throw new IllegalArgumentException("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
        }
        if (input.getBasepackage() == null) {
            throw new IllegalArgumentException("BlancoDbPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[basepackage]に値が設定されていません。");
        }
    }
}
